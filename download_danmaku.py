...
import time
from lib import ROOT, ASSETS, Encryptors, Keys
...
def download_them(l):
    os.makedirs(os.path.dirname(os.path.join(ROOT, l[0])), exist_ok=True)
    ...
    for category, info in json_obj["manifest"].items():
        ...
        for names in info:
            ...
            if assetdir not in filtering or names['AssetPath'] not in filtering[assetdir]\
                ...
                if names['AssetPath'].endswith('.dat'):
                    name_list.append((assetdir[1:]+names['AssetPath'], base_url+assetdir+names["HashedPath"],
